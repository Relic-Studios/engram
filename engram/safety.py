"""
engram.safety — Psychological safety systems.

Ported from Thomas-Soul's safety_manager and psychological_manager.
Covers influence/manipulation logging and psychological injury tracking.
"""
...
# ---------------------------------------------------------------------------
# Influence / manipulation logging
# ---------------------------------------------------------------------------
...
class InjuryTracker:
    ...
    def get_anchors(self) -> List[str]:
        ...
        Loads from ``anchors.yaml`` in the safety directory if present;
        ...
        if anchors_path.exists():
            try:
                ...
                log.warning("Failed to load anchors.yaml, using defaults: %s", exc)
        return list(self._DEFAULT_ANCHORS)
